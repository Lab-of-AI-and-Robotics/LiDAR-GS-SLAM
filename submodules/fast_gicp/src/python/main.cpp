...
PYBIND11_MODULE(pygicp, m) {
  ...
  py::class_<FastGICP, LsqRegistration, std::shared_ptr<FastGICP>>(m, "FastGICP")
    ...
    .def("get_target_control_score", [] (FastGICP& gicp){
      return py::array(gicp.getTargetControlSize(), gicp.getTargetControl().data());
      })
    ...
    .def("set_target_control_score", [] (FastGICP& gicp, py::array control_score){
      const auto input_control_score = control_score.cast<std::vector<float>>();
    	gicp.setTargetControl(input_control_score);
    })
    .def("set_target_filter", [] (FastGICP& gicp, py::int_ num_trackable, py::array filter){
      const auto input_filter = filter.cast<std::vector<int>>();
    	gicp.setTargetFilter(num_trackable, input_filter);
    })
    .def("set_source_intensities", [] (FastGICP& gicp, py::array intensities){
      const auto input_intensities = intensities.cast<std::vector<float>>();
    	gicp.setSourceIntensities(input_intensities);
    })
    .def("get_source_intensities", [] (FastGICP& gicp){
      return py::array(gicp.getSourceIntensitiesSize(), gicp.getSourceIntensities().data());
      })
  ;
