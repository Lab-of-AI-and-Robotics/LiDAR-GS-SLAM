...
template <typename PointSource, typename PointTarget, typename SearchMethodSource, typename SearchMethodTarget>
void FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::setTargetControl(const std::vector<float>& input_Control) {
  target_control_score.clear();
  target_control_score = input_Control;


}

template <typename PointSource, typename PointTarget, typename SearchMethodSource, typename SearchMethodTarget>
void FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::setSourceIntensities(
	const std::vector<float>& input_intensities)
	{
		source_intensities_ = input_intensities;
}
...
void FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  ...
  if (source_covs_.size() != input_->size()) {
    ...
    calculate_source_covariances_with_filter(input_, *search_source_, source_covs_, 
                                              source_rotationsq_, source_scales_, 
                                              source_filter_, source_intensities_);
  }
...
#pragma omp parallel for num_threads(num_threads_) reduction(+ : sum_errors) schedule(guided, 8)
  for (int i = 0; i < input_->size(); i++) {
    ...
    float ctr = target_control_score[target_index];
  
    float w = 1.0f / (ctr + 1e-3f);
    w = std::min(w, 10.0f);
    // if (target_control_score.size() == target_->size()) {
    //   w = 0.2f + 1.8f *(1-ctr); // control_score weight
    //   // w = 0.5f + 1.0f * (1.0f - ctr);   // 0.5 ~ 1.5 PCA ratio weight
    // }
    ...
    sum_errors += w*(error.transpose() * mahalanobis_[i] * error)(0,0);
...
bool FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::calculate_source_covariances_with_filter(
  ...
  // new_intensities.resize(cloud->size());
...
//    Eigen::Vector4d q = {qfrommat.x(), qfrommat.y(), qfrommat.z(), qfrommat.w()};
    ...
    Eigen::Vector3d normalVect;
    normalVect << 0, 0, 1;
    normalVect = svd.matrixU() * normalVect;
    Eigen::Vector3d pointVec;
    pointVec << cloud->at(i).x, cloud->at(i).y, cloud->at(i).z;
    // float cosalpha = pointVec.transpose() * normalVect / pointVec.norm();
    float cosalpha = abs(pointVec.dot(normalVect)) / pointVec.norm();
    intensities[i] = intensities[i] / std::max((float)0.7, cosalpha);
