...
def getProjectionMatrixFromK(znear, zfar, fx, fy, cx, cy, W, H):
    P = torch.zeros(4, 4)
    P[0, 0] = 2.0 * fx / W
    P[1, 1] = 2.0 * fy / H
    P[0, 2] = -2.0 * (cx / W) + 1.0
...
class SharedGaussians(nn.Module):
    def __init__(self, num_points):
        ...
        self.control_score = torch.zeros((num_points)).float().cuda()
        self.phys_conf = torch.zeros((num_points)).float().cuda()
        self.using_idx = torch.zeros((1)).int().cuda()
        self.filter_size = torch.zeros((1)).int().cuda()

    def input_values(self, new_xyz, new_rots, new_scales, new_z_values, new_trackable_filter, new_control_score, new_phys_conf):
        # on CPU memory
        ...
        n = min(int(self.using_idx[0]), int(new_control_score.shape[0]))
        if n > 0:
            self.control_score[:n] = new_control_score[:n]
        if int(self.using_idx[0]) > n:
            self.control_score[n:int(self.using_idx[0])] = 0
        n_phys = min(int(self.using_idx[0]), int(new_phys_conf.shape[0]))
        if n_phys > 0:
            self.phys_conf[:n_phys] = new_phys_conf[:n_phys]
        if int(self.using_idx[0]) > n_phys:
            self.phys_conf[n_phys:int(self.using_idx[0])] = 0
    def get_values(self):
        return  copy.deepcopy(self.xyz[:self.using_idx[0],:]),\
                ...
                copy.deepcopy(self.control_score[:self.using_idx[0]]),\
                copy.deepcopy(self.phys_conf[:self.using_idx[0]])
...
class SharedTargetPoints(nn.Module):
    def __init__(self, num_points):
        ...
        self.using_idx = torch.zeros((1)).int()
        self.control_score = torch.zeros((num_points)).float()

    def input_values(self, new_xyz, new_rots, new_scales, new_control_score):
        self.using_idx[0] = new_xyz.shape[0]
        ...
        self.rots[:self.using_idx[0],:] = new_rots
        self.scales[:self.using_idx[0],:] = new_scales
        self.control_score[:self.using_idx[0]] = new_control_score
    ...
    def get_values_np(self):
        return  copy.deepcopy(self.xyz[:self.using_idx[0],:].numpy()),\
                ...
                copy.deepcopy(self.control_score[:self.using_idx[0]].numpy())

class SharedCam(nn.Module):
    def __init__(self, FoVx, FoVy, depth_image,
                 cx, cy, fx, fy,
        ...
        # Use shape of depth_image for initialization
        self.image_width = torch.tensor([depth_image.shape[1]])
        self.image_height = torch.tensor([depth_image.shape[0]])
        ...
        self.original_normal_image = torch.zeros((3, depth_image.shape[0], depth_image.shape[1])).float()
    ...
    def update_matrix(self):
        ...
        self.projection_matrix[:3,:3] = self.K_tensor.transpose(0,1)
    
    def setup_cam(self, R, t, depth_img, K, normal_img=None):
        # Update pose.
        self.R[:,:] = torch.from_numpy(R)
        self.t[:] = torch.from_numpy(t)
        self.K_tensor[:, :] = torch.as_tensor(
            K, dtype=self.K_tensor.dtype, device=self.K_tensor.device
        ) ## Feature_LGS
        # Update intrinsics from K.
        self.fx[0] = self.K_tensor[0, 0]
        self.fy[0] = self.K_tensor[1, 1]
        self.cx[0] = self.K_tensor[0, 2]
        self.cy[0] = self.K_tensor[1, 2]

        # Update images.
        self.original_depth_image[:,:,:] = torch.from_numpy(depth_img).float().unsqueeze(0)
        if normal_img is not None:
            self.original_normal_image[:,:,:] = torch.from_numpy(normal_img).float()

        # Refresh pose and projection matrices.
        self.update_matrix()
    ...
    def on_cuda(self):
        ...
        self.original_normal_image = self.original_normal_image.cuda()
