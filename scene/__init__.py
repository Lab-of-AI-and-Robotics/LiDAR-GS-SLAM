...
class Scene(nn.Module):
    ...
    def __init__(self, args : ModelParams, load_iteration=None, shuffle=True, resolution_scales=[1.0], slam_trigger=False):
        ...
        # Modified: Disabled legacy loader logic for SLAM refactoring
        """
        if self.slam:
            print("SLAM mode")
        ...
        """
        # Placeholder for scene_info to avoid crashes if this class is instantiated
        scene_info = None
        print("Warning: Scene class legacy loader is disabled.")

        if scene_info and not self.loaded_iter:
            if not self.slam:
