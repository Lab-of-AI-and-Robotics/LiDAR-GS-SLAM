...
def render(
        camera: SharedCam,
        model: GaussianModel,
        depth_ratio: float = 0.0, gaussian_indices = None):
    ...
    if gaussian_indices is None:
        ...
        means3D = model.get_xyz[gaussian_indices]
        opacities = model.get_opacity[gaussian_indices]
        scales = model.get_scaling[gaussian_indices]
        rotations = model.get_rotation[gaussian_indices]
        if scales.shape[1] == 3:
            scales = scales[:, :2].contiguous()

    means2D = torch.zeros_like(means3D,dtype=torch.float32, device=means3D.device)
