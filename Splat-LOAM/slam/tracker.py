...
class Tracker:
    ...
    @torch.no_grad()
    def register_keyframe(self, keyframe: Frame, local_gaussians_mean, T) -> None:
        """
        ...
        self.aligner.set_target(keyframe, local_gaussians_mean, T)
        self.num_frames_tracked = 0
...
class Aligner(Protocol):
    ...
    def set_target(self, frame: Frame, local_gaussians_mean, T) -> None:
...
class AlignerGeomPhoto:
    ...
    def set_target(self, frame: Frame, local_gaussians_mean, local_map_T) -> None:
        """ Setup the target frame (typically the fixed keyframe) """
        assert self.model is not None
        ...
        R = frame.world_T_frame[:3, :3]
        t = frame.world_T_frame[:3, 3]

        # 1. t를 (3,)에서 (3, 1)로 변경
        t_col = t.unsqueeze(1) 
        # 2. points를 (N, 3)에서 (3, N)으로 변경
        points_T = points.T

        # 3. p_w = R @ p_l + t
        points_world_T = R @ points_T + t_col # (3, N)

        # 4. (3, N) -> (N, 3)로 다시 전치
        points_vis = points_world_T.T

        rr.log(
            "world/target_points",
            rr.Points3D(
                positions=points_vis.detach().cpu().numpy()
            )
        )

        if local_gaussians_mean != None:
            # R = local_map_T[:3,:3]
            # T = local_map_T[:3,3]

            local_gaussians_mean=(R@local_gaussians_mean.T + t_col).T

            print(points.shape, local_gaussians_mean.shape)

            rr.log(
                "world/target_gs_raw",
                rr.Points3D(
                    positions=local_gaussians_mean.detach().cpu().numpy()
                )
            )

        #     self.gsaligner.set_reference(
        #         depth, local_gaussians_mean, frame.camera.projection_matrix)
        # else:
        #     self.gsaligner.set_reference(
