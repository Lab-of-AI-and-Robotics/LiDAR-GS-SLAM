...
class SLAM:
    ...
    def insert_new_keyframe(self, frame: Frame):
        ...
        local_gaussians_mean = self.mapper.model.get_gmodel.get_xyz
        local_map_T = self.mapper.model.world_T_model

        self.tracker.register_keyframe(frame, local_gaussians_mean, local_map_T)
        with torch.no_grad():
    ...
    def initialize_new_local_model(self, frame: Frame):
        ...
        self.tracker.register_keyframe(frame, None, None)
        self.frames.append(frame)
        with torch.no_grad():
            get_datalogger(self.cfg).log_model(
                "world/model", self.local_models[-1].get_gmodel)
